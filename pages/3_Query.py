...
OUT_SCANS_CSV = f"{OUT_PATH}/scans.csv"
...
SELECT * FROM read_csv_auto('{OUT_SCANS_CSV}')
""")
...
ON {left_alias}.{join_key} = {alias}.{join_key}"""
    ...
    LIMIT 1
        ...
        elif date_mode == "Nearest to scan/reference date":
...
# ---------- Create pivot views ----------
if "event" in get_columns("events"):
    make_pivot_view(
        source_table="events",
...
elif "event_type" in get_columns("events"):
    make_pivot_view(
        source_table="events",
        pivot_view_name="events_pivot",
        id_col="patient_id",
...
for i, join in enumerate(st.session_state.joins):
    ...
    with col1:
        ...
        join["date_mode"] = st.selectbox(
            "Date filter",
            ["All rows", "Latest", "Nearest to scan/reference date"],
            index=["All rows", "Latest", "Nearest to scan/reference date"].index(join["date_mode"])
            if join["date_mode"] in ["All rows", "Latest", "Nearest to scan/reference date"] else 0,
            key=f"date_mode_{i}",
        )
    ...
    with col2:
        ...
        if join["date_mode"] == "Nearest to scan/reference date":
    ...
    with col3:
        if join["date_mode"] in ["Latest", "Nearest to scan/reference date"]:
